...
CrewRank:add_bare_item("", function()
...
DefNum33Cur = 1
CrewRank:add_array_item("Number #1", NumberList2, function()
	return DefNum33Cur
end, function(NewNum33)
...
DefNum34Cur = 1
CrewRank:add_array_item("Number #2", NumberList1, function()
	return DefNum34Cur
end, function(NewNum34)
...
DefNum35Cur = 1
CrewRank:add_array_item("Number #3", NumberList1, function()
	return DefNum35Cur
end, function(NewNum35)
...
DefNum36Cur = 1
CrewRank:add_array_item("Number #4", NumberList1, function()
	return DefNum36Cur
end, function(NewNum36)
...
a78 = true
CrewRank:add_toggle("Reset Value", function()
	return a78
end, function()
	a78 = not a78
end)
...
AWRMenu:add_action("Unlock Arena War Bools", function()
	for c = 0, 63 do
		...
		stats.set_bool_masked(MPX .. "ARENAWARSPSTAT_BOOL2", true, c)
		stats.set_bool_masked(MPX .. "ARENAWARSPSTAT_BOOL3", true, c)
		stats.set_bool_masked(MPX .. "ARENAWARSPSTAT_BOOL4", true, c)
		stats.set_bool_masked(MPX .. "ARENAWARSPSTAT_BOOL5", true, c)
		stats.set_bool_masked(MPX .. "ARENAWARSPSTAT_BOOL6", true, c)
		stats.set_bool_masked(MPX .. "ARENAWARSPSTAT_BOOL7", true, c)
		stats.set_bool_masked(MPX .. "ARENAWARSPSTAT_BOOL8", true, c)
	end
end)
...
RONECLICKMenu:add_action("Unlock Oppressor MKII Trade Price", function()
...
OnlineServices = L7NEG:add_submenu("Online Services")

CasinoServices = OnlineServices:add_submenu("Casino Services")
...
b26 = 1
CasinoServices:add_array_item("⚠ Get Lucky Wheel Prize", prizes_name, function()
	return b26
end, function(GivLucPri)
	b26 = GivLucPri
	CLW:set_int(CMGLPl1, prizes_id[b26])
	CLW:set_int(CMGLPl2, 11)
	b26 = 1
end)

CasinoServices:add_action("⚠ Rig Slot Machines", function()
	for i = 3, 196 do
		if i ~= 67 and i ~= 132 then
			CS:set_int(CMSRRTl + i, 6)
		end
	end
end)
...
-- This script was made & developed by Goose. Please do not reproduce the script under your name.--------------------------------------------------------------------------------------------------------------------
...
local CarSubCompacts = CarDirectory:add_submenu("Compacts | 10")
local CarSubCoupes = CarDirectory:add_submenu("Coupes | 16")
local CarSubCycles = CarDirectory:add_submenu("Cycles | 7")
local CarSubEmergency = CarDirectory:add_submenu("Emergency | 22")
local CarSubHelicopters = CarDirectory:add_submenu("Helicopters | 20")
local CarSubIndustrial = CarDirectory:add_submenu("Industrial | 11")
local CarSubMilitary = CarDirectory:add_submenu("Military | 8")
local CarSubMotorcycles = CarDirectory:add_submenu("Motorcycles | 48")
local CarSubMuscle = CarDirectory:add_submenu("Muscle | 49")
local CarSubOffRoad = CarDirectory:add_submenu("Off-Road | 38")
local CarSubPlanes = CarDirectory:add_submenu("Planes | 21")
local CarSubSedans = CarDirectory:add_submenu("Sedans | 28")
local CarSubService = CarDirectory:add_submenu("Service | 11")
local CarSubSports = CarDirectory:add_submenu("Sports | 61")
local CarSubSUV = CarDirectory:add_submenu("SUVs | 27")
local CarSubUtility = CarDirectory:add_submenu("Utility | 20")
local CarSubVans = CarDirectory:add_submenu("Vans | 35")
local CarSubCommercial = CarDirectory:add_submenu("Commercial | 16")
local CarSubSuperVehicles = CarDirectory:add_submenu("Supervehicles | 4")
local CarSubSpecial = CarDirectory:add_submenu("Special Vehicles | 3")
local CarSubSaves = CarDirectory:add_submenu("Saved Vehicles")
local Other = CarDirectory:add_submenu("Other Options")
local Credits = Other:add_submenu("Credits")
local Logs = Other:add_submenu("Logs")
Logs:add_action("Ultimate Vehicle Spawner | Loaded", function()
	sleep(0.1)
end)
-- MAIN SYSTEM ()
...
function SaveVehicle()
	...
	CarSubSaves:add_action("Saved Vehicle #" .. SavedVehicles .. "| " .. CurrentHash, function()
		SpawnVehicle(CurrentHash)
	end)
	Logs:add_action("Vehicle Saved | " .. CurrentHash, function()
		SpawnVehicle(CurrentHash)
	end)
end

function SaveNearestVehicle()
	vehicle = localplayer:get_nearest_vehicle()
	CurrentHash = vehicle:get_model_hash()
	SavedVehicles = SavedVehicles + 1
	CarSubSaves:add_action("Saved Nearest Vehicle #" .. SavedVehicles .. "| " .. CurrentHash, function()
		SpawnVehicle(CurrentHash)
	end)
	Logs:add_action("Vehicle Saved | " .. CurrentHash, function()
		SpawnVehicle(CurrentHash)
...
-- Saves
CarSubSaves:add_action("Save Vehicle", function()
	SaveVehicle()
end)
CarSubSaves:add_action("Save Nearest Vehicle", function()
	SaveNearestVehicle()
end)
-- Credits
Credits:add_action("Goose | Developer", function()
	sleep(1)
end)
Credits:add_action("Kíllà'B | Debug Help", function()
	sleep(1)
end)
Credits:add_action("Bababoiiiii | Debug Help", function()
	sleep(1)
end)
Credits:add_action("mopsfl. | fixed the hash number xd", function()
	sleep(1)
end)
Credits:add_action("L7NEG | 1.68 Globals And New Dlc Vehicles", function()
	sleep(1)
end)

-- Supervehicles
CarSubSuperVehicles:add_action("Spawn Avenger", function()
	menu.deliver_avenger()
end)
CarSubSuperVehicles:add_action("Spawn Kosatska", function()
	menu.deliver_kosatka()
end)
CarSubSuperVehicles:add_action("Spawn Mobile Command Center", function()
	menu.deliver_moc()
end)
CarSubSuperVehicles:add_action("Spawn Terrorbyte", function()
	menu.deliver_terrorbyte()
end)

-- Special
CarSubSpecial:add_action("Deliver Kosatka Avisa Submarine", function()
	menu.deliver_avisa()
end)
CarSubSpecial:add_action("Deliver Kosatka Dinghy", function()
	menu.deliver_dinghy()
end)
CarSubSpecial:add_action("Deliver Kostaka Sea Sparrow", function()
	menu.deliver_sparrow()
end)

-- Boats
CarSubBoat:add_action("Spawn Dinghy1", function()
	SpawnVehicle(1033245328)
end)
CarSubBoat:add_action("Spawn Dinghy2", function()
	SpawnVehicle(276773164)
end)
CarSubBoat:add_action("Spawn Dinghy3", function()
	SpawnVehicle(509498602)
end)
CarSubBoat:add_action("Spawn Dinghy4", function()
	SpawnVehicle(867467158)
end)
CarSubBoat:add_action("Spawn Jetmax", function()
	SpawnVehicle(861409633)
end)
CarSubBoat:add_action("Spawn Marquis", function()
	SpawnVehicle(-1043459709)
end)
CarSubBoat:add_action("Spawn SeaShark", function()
	SpawnVehicle(-1030275036)
end)
CarSubBoat:add_action("Spawn SeaShark2", function()
	SpawnVehicle(-616331036)
end)
CarSubBoat:add_action("Spawn SeaShark3", function()
	SpawnVehicle(-311022263)
end)
CarSubBoat:add_action("Spawn Speeder", function()
	SpawnVehicle(231083307)
end)
CarSubBoat:add_action("Spawn Speeder2", function()
	SpawnVehicle(437538602)
end)
CarSubBoat:add_action("Spawn Squalo", function()
	SpawnVehicle(400514754)
end)
CarSubBoat:add_action("Spawn Submersible", function()
	SpawnVehicle(771711535)
end)
CarSubBoat:add_action("Spawn Submersible2", function()
	SpawnVehicle(-1066334226)
end)
CarSubBoat:add_action("Spawn Suntrap", function()
	SpawnVehicle(-282946103)
end)
CarSubBoat:add_action("Spawn Toro", function()
	SpawnVehicle(1070967343)
end)
CarSubBoat:add_action("Spawn Toro2", function()
	SpawnVehicle(908897389)
end)
CarSubBoat:add_action("Spawn Tropic", function()
	SpawnVehicle(290013743)
end)
CarSubBoat:add_action("Spawn Tropic2", function()
	SpawnVehicle(1448677353)
end)
CarSubBoat:add_action("Spawn Tug", function()
	SpawnVehicle(-2100640717)
end)

-- Compacts
CarSubCompacts:add_action("Spawn Blista", function()
	SpawnVehicle(-344943009)
end)
CarSubCompacts:add_action("Spawn Blista2", function()
	SpawnVehicle(1039032026)
end)
CarSubCompacts:add_action("Spawn Blista3", function()
	SpawnVehicle(-591651781)
end)
CarSubCompacts:add_action("Spawn Brioso", function()
	SpawnVehicle(1549126457)
end)
CarSubCompacts:add_action("Spawn Dilettante", function()
	SpawnVehicle(-1130810103)
end)
CarSubCompacts:add_action("Spawn Dilettante2", function()
	SpawnVehicle(1682114128)
end)
CarSubCompacts:add_action("Spawn Issi2", function()
	SpawnVehicle(-1177863319)
end)
CarSubCompacts:add_action("Spawn Panto", function()
	SpawnVehicle(-431692672)
end)
CarSubCompacts:add_action("Spawn Prairie", function()
	SpawnVehicle(-1450650718)
end)
CarSubCompacts:add_action("Spawn Rhapsody", function()
	SpawnVehicle(841808271)
end)

-- Coupes
CarSubCoupes:add_action("Spawn CogCabrio", function()
	SpawnVehicle(330661258)
end)
CarSubCoupes:add_action("Spawn Exemplar", function()
	SpawnVehicle(-5153954)
end)
CarSubCoupes:add_action("Spawn F620", function()
	SpawnVehicle(-591610296)
end)
CarSubCoupes:add_action("Spawn Felon", function()
	SpawnVehicle(-391594584)
end)
CarSubCoupes:add_action("Spawn Felon2", function()
	SpawnVehicle(-89291282)
end)
CarSubCoupes:add_action("Spawn Jackal", function()
	SpawnVehicle(-624529134)
end)
CarSubCoupes:add_action("Spawn Kanjo", function()
	SpawnVehicle(64075878)
end)
CarSubCoupes:add_action("Spawn Oracle", function()
	SpawnVehicle(1348744438)
end)
CarSubCoupes:add_action("Spawn Oracle2", function()
	SpawnVehicle(-511601230)
end)
CarSubCoupes:add_action("Spawn Postlude", function()
	SpawnVehicle(294678663)
end)
CarSubCoupes:add_action("Spawn Sentinel", function()
	SpawnVehicle(1349725314)
end)
CarSubCoupes:add_action("Spawn Sentinel2", function()
	SpawnVehicle(873639469)
end)
CarSubCoupes:add_action("Spawn Windsor", function()
	SpawnVehicle(1581459400)
end)
CarSubCoupes:add_action("Spawn Windsor2", function()
	SpawnVehicle(1930048799)
end)
CarSubCoupes:add_action("Spawn Zion", function()
	SpawnVehicle(-1122289213)
end)
CarSubCoupes:add_action("Spawn Zion2", function()
	SpawnVehicle(-1193103848)
end)

-- Cycles
CarSubCycles:add_action("Spawn BMX", function()
	SpawnVehicle(1131912276)
end)
CarSubCycles:add_action("Spawn Cruiser", function()
	SpawnVehicle(448402357)
end)
CarSubCycles:add_action("Spawn Fixter", function()
	SpawnVehicle(-836512833)
end)
CarSubCycles:add_action("Spawn Scorcher", function()
	SpawnVehicle(-186537451)
end)
CarSubCycles:add_action("Spawn TriBike", function()
	SpawnVehicle(1127861609)
end)
CarSubCycles:add_action("Spawn TriBike2", function()
	SpawnVehicle(-1233807380)
end)
CarSubCycles:add_action("Spawn TriBike3", function()
	SpawnVehicle(-400295096)
end)

-- Emergency
CarSubEmergency:add_action("Spawn Ambulance", function()
	SpawnVehicle(1171614426)
end)
CarSubEmergency:add_action("Spawn FBI", function()
	SpawnVehicle(1127131465)
end)
CarSubEmergency:add_action("Spawn FBI2", function()
	SpawnVehicle(-1647941228)
end)
CarSubEmergency:add_action("Spawn FireTruck", function()
	SpawnVehicle(1938952078)
end)
CarSubEmergency:add_action("Spawn PrisonBus", function()
	SpawnVehicle(-2007026063)
end)
CarSubEmergency:add_action("Spawn Police", function()
	SpawnVehicle(2046537925)
end)
CarSubEmergency:add_action("Spawn Police2", function()
	SpawnVehicle(-1627000575)
end)
CarSubEmergency:add_action("Spawn Police3", function()
	SpawnVehicle(1912215274)
end)
CarSubEmergency:add_action("Spawn Police4", function()
	SpawnVehicle(-1973172295)
end)
CarSubEmergency:add_action("Spawn PoliceOld1", function()
	SpawnVehicle(-1536924937)
end)
CarSubEmergency:add_action("Spawn PoliceOld2", function()
	SpawnVehicle(-1779120616)
end)
CarSubEmergency:add_action("Spawn PoliceTransport", function()
	SpawnVehicle(456714581)
end)
CarSubEmergency:add_action("Spawn PoliceBike", function()
	SpawnVehicle(-34623805)
end)
CarSubEmergency:add_action("Spawn PoliceHelicopter", function()
	SpawnVehicle(353883353)
end)
CarSubEmergency:add_action("Spawn ParkRanger", function()
	SpawnVehicle(741586030)
end)
CarSubEmergency:add_action("Spawn Predator", function()
	SpawnVehicle(-488123221)
end)
CarSubEmergency:add_action("Spawn Riot", function()
	SpawnVehicle(-1205689942)
end)
CarSubEmergency:add_action("Spawn Sheriff", function()
	SpawnVehicle(-1683328900)
end)
CarSubEmergency:add_action("Spawn Sheriff2", function()
	SpawnVehicle(1922257928)
end)
CarSubEmergency:add_action("Spawn Gauntlet Interceptor", function()
	SpawnVehicle(-1233767450)
end)
CarSubEmergency:add_action("Spawn Police Riot", function()
	SpawnVehicle(-1205689942)
end)
CarSubEmergency:add_action("Spawn LE Cruiser", function()
	SpawnVehicle(-1674384553)
end)

-- Helicopters
CarSubHelicopters:add_action("Spawn Annihilator", function()
	SpawnVehicle(837858166)
end)
CarSubHelicopters:add_action("Spawn Buzzard", function()
	SpawnVehicle(788747387)
end)
CarSubHelicopters:add_action("Spawn Buzzard2", function()
	SpawnVehicle(745926877)
end)
CarSubHelicopters:add_action("Spawn Cargobob", function()
	SpawnVehicle(-50547061)
end)
CarSubHelicopters:add_action("Spawn Cargobob2", function()
	SpawnVehicle(1621617168)
end)
CarSubHelicopters:add_action("Spawn Cargobob3", function()
	SpawnVehicle(1394036463)
end)
CarSubHelicopters:add_action("Spawn Cargobob4", function()
	SpawnVehicle(2025593404)
end)
CarSubHelicopters:add_action("Spawn Canoda", function()
	SpawnVehicle(-477831899)
end)
CarSubHelicopters:add_action("Spawn Frogger", function()
	SpawnVehicle(744705981)
end)
CarSubHelicopters:add_action("Spawn Frogger2", function()
	SpawnVehicle(1949211328)
end)
CarSubHelicopters:add_action("Spawn Maverick", function()
	SpawnVehicle(-1660661558)
end)
CarSubHelicopters:add_action("Spawn Savage", function()
	SpawnVehicle(-82626025)
end)
CarSubHelicopters:add_action("Spawn Skylift", function()
	SpawnVehicle(1044954915)
end)
CarSubHelicopters:add_action("Spawn Supervolito", function()
	SpawnVehicle(710198397)
end)
CarSubHelicopters:add_action("Spawn Supervolito2", function()
	SpawnVehicle(-1671539132)
end)
CarSubHelicopters:add_action("Spawn Swift", function()
	SpawnVehicle(-339587598)
end)
CarSubHelicopters:add_action("Spawn Swift2", function()
	SpawnVehicle(1075432268)
end)
CarSubHelicopters:add_action("Spawn Valkyrie", function()
	SpawnVehicle(-1600252419)
end)
CarSubHelicopters:add_action("Spawn Valkyrie2", function()
	SpawnVehicle(1543134283)
end)
CarSubHelicopters:add_action("Spawn Volatus", function()
	SpawnVehicle(-1845487887)
end)

-- Industrial
CarSubIndustrial:add_action("Spawn Bulldozer", function()
	SpawnVehicle(1886712733)
end)
CarSubIndustrial:add_action("Spawn Cutter", function()
	SpawnVehicle(-1006919392)
end)
CarSubIndustrial:add_action("Spawn Dump", function()
	SpawnVehicle(-2130482718)
end)
CarSubIndustrial:add_action("Spawn Flatbed", function()
	SpawnVehicle(1353720154)
end)
CarSubIndustrial:add_action("Spawn Guardian", function()
	SpawnVehicle(-2107990196)
end)
CarSubIndustrial:add_action("Spawn Handler", function()
	SpawnVehicle(444583674)
end)
CarSubIndustrial:add_action("Spawn Mixer", function()
	SpawnVehicle(-784816453)
end)
CarSubIndustrial:add_action("Spawn Mixer2", function()
	SpawnVehicle(475220373)
end)
CarSubIndustrial:add_action("Spawn Rubble", function()
	SpawnVehicle(-1705304628)
end)
CarSubIndustrial:add_action("Spawn TipTruck", function()
	SpawnVehicle(48339065)
end)
CarSubIndustrial:add_action("Spawn TipTruck2", function()
	SpawnVehicle(-947761570)
end)

-- Military
CarSubMilitary:add_action("Spawn APC", function()
	SpawnVehicle(562680400)
end)
CarSubMilitary:add_action("Spawn Barracks", function()
	SpawnVehicle(-823509173)
end)
CarSubMilitary:add_action("Spawn Barracks2", function()
	SpawnVehicle(1074326203)
end)
CarSubMilitary:add_action("Spawn Barracks3", function()
	SpawnVehicle(630371791)
end)
CarSubMilitary:add_action("Spawn Crusader", function()
	SpawnVehicle(321739290)
end)
CarSubMilitary:add_action("Spawn Halftrack", function()
	SpawnVehicle(-32236122)
end)
CarSubMilitary:add_action("Spawn Rhino", function()
	SpawnVehicle(782665360)
end)
CarSubMilitary:add_action("Spawn Trailersmall2", function()
	SpawnVehicle(-1881846085)
end)

-- Motorcycles
CarSubMotorcycles:add_action("Spawn Akuma", function()
	SpawnVehicle(1672195559)
end)
CarSubMotorcycles:add_action("Spawn Avarus", function()
	SpawnVehicle(-2115793025)
end)
CarSubMotorcycles:add_action("Spawn Bagger", function()
	SpawnVehicle(-2140431165)
end)
CarSubMotorcycles:add_action("Spawn Bati2", function()
	SpawnVehicle(-891462355)
end)
CarSubMotorcycles:add_action("Spawn Bati", function()
	SpawnVehicle(-114291515)
end)
CarSubMotorcycles:add_action("Spawn BF400", function()
	SpawnVehicle(86520421)
end)
CarSubMotorcycles:add_action("Spawn Blazer4", function()
	SpawnVehicle(-440768424)
end)
CarSubMotorcycles:add_action("Spawn CarbonRS", function()
	SpawnVehicle(11251904)
end)
CarSubMotorcycles:add_action("Spawn Chimera", function()
	SpawnVehicle(6774487)
end)
CarSubMotorcycles:add_action("Spawn Cliffhanger", function()
	SpawnVehicle(390201602)
end)
CarSubMotorcycles:add_action("Spawn Daemon2", function()
	SpawnVehicle(-1404136503)
end)
CarSubMotorcycles:add_action("Spawn Daemon", function()
	SpawnVehicle(2006142190)
end)
CarSubMotorcycles:add_action("Spawn Defiler", function()
	SpawnVehicle(822018448)
end)
CarSubMotorcycles:add_action("Spawn Double", function()
	SpawnVehicle(-1670998136)
end)
CarSubMotorcycles:add_action("Spawn Enduro", function()
	SpawnVehicle(1753414259)
end)
CarSubMotorcycles:add_action("Spawn Esskey", function()
	SpawnVehicle(2035069708)
end)
CarSubMotorcycles:add_action("Spawn Faggio", function()
	SpawnVehicle(-1842748181)
end)
CarSubMotorcycles:add_action("Spawn Faggio2", function()
	SpawnVehicle(55628203)
end)
CarSubMotorcycles:add_action("Spawn Faggio3", function()
	SpawnVehicle(-1289178744)
end)
CarSubMotorcycles:add_action("Spawn Fcr2", function()
	SpawnVehicle(-757735410)
end)
CarSubMotorcycles:add_action("Spawn Fcr", function()
	SpawnVehicle(627535535)
end)
CarSubMotorcycles:add_action("Spawn Gargoyle", function()
	SpawnVehicle(741090084)
end)
CarSubMotorcycles:add_action("Spawn Hakuchou2", function()
	SpawnVehicle(-255678177)
end)
CarSubMotorcycles:add_action("Spawn Hakuchou", function()
	SpawnVehicle(1265391242)
end)
CarSubMotorcycles:add_action("Spawn Hexer", function()
	SpawnVehicle(301427732)
end)
CarSubMotorcycles:add_action("Spawn Innovation", function()
	SpawnVehicle(-159126838)
end)
CarSubMotorcycles:add_action("Spawn Lectro", function()
	SpawnVehicle(640818791)
end)
CarSubMotorcycles:add_action("Spawn Manchez", function()
	SpawnVehicle(-1523428744)
end)
CarSubMotorcycles:add_action("Spawn Manchez2", function()
	SpawnVehicle(1384502824)
end)
CarSubMotorcycles:add_action("Spawn Nemesis", function()
	SpawnVehicle(-634879114)
end)
CarSubMotorcycles:add_action("Spawn Nightblade", function()
	SpawnVehicle(-1606187161)
end)
CarSubMotorcycles:add_action("Spawn Oppressor", function()
	SpawnVehicle(884483972)
end)
CarSubMotorcycles:add_action("Spawn OppressorMK2", function()
	SpawnVehicle(884483972)
end)
CarSubMotorcycles:add_action("Spawn PCJ", function()
	SpawnVehicle(-909201658)
end)
CarSubMotorcycles:add_action("Spawn Ratbike", function()
	SpawnVehicle(1873600305)
end)
CarSubMotorcycles:add_action("Spawn Ruffian", function()
	SpawnVehicle(-893578776)
end)
CarSubMotorcycles:add_action("Spawn Sanchez2", function()
	SpawnVehicle(-1453280962)
end)
CarSubMotorcycles:add_action("Spawn Sanchez", function()
	SpawnVehicle(788045382)
end)
CarSubMotorcycles:add_action("Spawn Sanctus", function()
	SpawnVehicle(1491277511)
end)
CarSubMotorcycles:add_action("Spawn Shotaro", function()
	SpawnVehicle(-405626514)
end)
CarSubMotorcycles:add_action("Spawn Sovereign", function()
	SpawnVehicle(743478836)
end)
CarSubMotorcycles:add_action("Spawn Thrust", function()
	SpawnVehicle(1836027715)
end)
CarSubMotorcycles:add_action("Spawn Vader", function()
	SpawnVehicle(-140902153)
end)
CarSubMotorcycles:add_action("Spawn Vindicator", function()
	SpawnVehicle(-1353081087)
end)
CarSubMotorcycles:add_action("Spawn Vortex", function()
	SpawnVehicle(-609625092)
end)
CarSubMotorcycles:add_action("Spawn Wolfsbane", function()
	SpawnVehicle(-618617997)
end)
CarSubMotorcycles:add_action("Spawn Zombiea", function()
	SpawnVehicle(-1009268949)
end)
CarSubMotorcycles:add_action("Spawn Zombieb", function()
	SpawnVehicle(-570033273)
end)

-- Muscle
CarSubMuscle:add_action("Spawn Blade", function()
	SpawnVehicle(-1205801634)
end)
CarSubMuscle:add_action("Spawn Buccaneer", function()
	SpawnVehicle(-682211828)
end)
CarSubMuscle:add_action("Spawn Buccaneer2", function()
	SpawnVehicle(-1013450936)
end)
CarSubMuscle:add_action("Spawn Broadway", function()
	SpawnVehicle(-1933242328)
end)
CarSubMuscle:add_action("Spawn Chino", function()
	SpawnVehicle(349605904)
end)
CarSubMuscle:add_action("Spawn Chino2", function()
	SpawnVehicle(-1361687965)
end)
CarSubMuscle:add_action("Spawn Dominator", function()
	SpawnVehicle(80636076)
end)
CarSubMuscle:add_action("Spawn Dominator2", function()
	SpawnVehicle(-915704871)
end)
CarSubMuscle:add_action("Spawn Dukes", function()
	SpawnVehicle(723973206)
end)
CarSubMuscle:add_action("Spawn Dukes2", function()
	SpawnVehicle(-326143852)
end)
CarSubMuscle:add_action("Spawn Eudora", function()
	SpawnVehicle(-1249788006)
end)
CarSubMuscle:add_action("Spawn Faction", function()
	SpawnVehicle(-2119578145)
end)
CarSubMuscle:add_action("Spawn Faction2", function()
	SpawnVehicle(-1790546981)
end)
CarSubMuscle:add_action("Spawn Faction3", function()
	SpawnVehicle(-2039755226)
end)
CarSubMuscle:add_action("Spawn Gauntlet", function()
	SpawnVehicle(-1800170043)
end)
CarSubMuscle:add_action("Spawn Gauntlet2", function()
	SpawnVehicle(349315417)
end)
CarSubMuscle:add_action("Spawn Greenwood", function()
	SpawnVehicle(40817712)
end)
CarSubMuscle:add_action("Spawn Hotknife", function()
	SpawnVehicle(37348240)
end)
CarSubMuscle:add_action("Spawn Ruinerzz8", function()
	SpawnVehicle(51706945532)
end)
CarSubMuscle:add_action("Spawn Lurcher", function()
	SpawnVehicle(2068293287)
end)
CarSubMuscle:add_action("Spawn Moonbeam", function()
	SpawnVehicle(525509695)
end)
CarSubMuscle:add_action("Spawn Moonbeam2", function()
	SpawnVehicle(1896491931)
end)
CarSubMuscle:add_action("Spawn Nightshade", function()
	SpawnVehicle(-1943285540)
end)
CarSubMuscle:add_action("Spawn Phoenix", function()
	SpawnVehicle(-2095439403)
end)
CarSubMuscle:add_action("Spawn Picador", function()
	SpawnVehicle(1507916787)
end)
CarSubMuscle:add_action("Spawn RatLoader", function()
	SpawnVehicle(-667151410)
end)
CarSubMuscle:add_action("Spawn RatLoader2", function()
	SpawnVehicle(-589178377)
end)
CarSubMuscle:add_action("Spawn Ruiner", function()
	SpawnVehicle(-227741703)
end)
CarSubMuscle:add_action("Spawn Ruiner2", function()
	SpawnVehicle(941494461)
end)
CarSubMuscle:add_action("Spawn SabreGT", function()
	SpawnVehicle(-1685021548)
end)
CarSubMuscle:add_action("Spawn SabreGT2", function()
	SpawnVehicle(223258115)
end)
CarSubMuscle:add_action("Spawn Sadler2", function()
	SpawnVehicle(734217681)
end)
CarSubMuscle:add_action("Spawn SlamVan", function()
	SpawnVehicle(729783779)
end)
CarSubMuscle:add_action("Spawn SlamVan2", function()
	SpawnVehicle(833469436)
end)
CarSubMuscle:add_action("Spawn SlamVan3", function()
	SpawnVehicle(1119641113)
end)
CarSubMuscle:add_action("Spawn Stalion", function()
	SpawnVehicle(1923400478)
end)
CarSubMuscle:add_action("Spawn Stalion2", function()
	SpawnVehicle(-401643538)
end)
CarSubMuscle:add_action("Spawn Tampa", function()
	SpawnVehicle(972671128)
end)
CarSubMuscle:add_action("Spawn Tampa3", function()
	SpawnVehicle(-1210451983)
end)
CarSubMuscle:add_action("Spawn Tahoma", function()
	SpawnVehicle(461850249)
end)
CarSubMuscle:add_action("Spawn Tulip", function()
	SpawnVehicle(268758436)
end)
CarSubMuscle:add_action("Spawn Vigero", function()
	SpawnVehicle(-825837129)
end)
CarSubMuscle:add_action("Spawn Vigero2", function()
	SpawnVehicle(1758379524)
end)
CarSubMuscle:add_action("Spawn Virgo", function()
	SpawnVehicle(-498054846)
end)
CarSubMuscle:add_action("Spawn Virgo2", function()
	SpawnVehicle(-899509638)
end)
CarSubMuscle:add_action("Spawn Virgo3", function()
	SpawnVehicle(16646064)
end)
CarSubMuscle:add_action("Spawn Voodoo", function()
	SpawnVehicle(2006667053)
end)
CarSubMuscle:add_action("Spawn Voodoo2", function()
	SpawnVehicle(523724515)
end)
CarSubMuscle:add_action("Spawn Weevil", function()
	SpawnVehicle(1644055914)
end)

-- OffRoad
CarSubOffRoad:add_action("Spawn BfInjection", function()
	SpawnVehicle(1126868326)
end)
CarSubOffRoad:add_action("Spawn Bifta", function()
	SpawnVehicle(-349601129)
end)
CarSubOffRoad:add_action("Spawn Blazer", function()
	SpawnVehicle(-2128233223)
end)
CarSubOffRoad:add_action("Spawn Blazer2", function()
	SpawnVehicle(-48031959)
end)
CarSubOffRoad:add_action("Spawn Blazer3", function()
	SpawnVehicle(-1269889662)
end)
CarSubOffRoad:add_action("Spawn Blazer5", function()
	SpawnVehicle(-1590337689)
end)
CarSubOffRoad:add_action("Spawn Bodhi2", function()
	SpawnVehicle(-1435919434)
end)
CarSubOffRoad:add_action("Spawn Brawler", function()
	SpawnVehicle(-1479664699)
end)
CarSubOffRoad:add_action("Spawn Boar", function()
	SpawnVehicle(996383885)
end)
CarSubOffRoad:add_action("Spawn DLoader", function()
	SpawnVehicle(1770332643)
end)
CarSubOffRoad:add_action("Spawn Dune", function()
	SpawnVehicle(-1661854193)
end)
CarSubOffRoad:add_action("Spawn Dune2", function()
	SpawnVehicle(534258863)
end)
CarSubOffRoad:add_action("Spawn Dune3", function()
	SpawnVehicle(1897744184)
end)
CarSubOffRoad:add_action("Spawn Dune4", function()
	SpawnVehicle(-827162039)
end)
CarSubOffRoad:add_action("Spawn Dune5", function()
	SpawnVehicle(-312295511)
end)
CarSubOffRoad:add_action("Spawn Drauger", function()
	SpawnVehicle(768236378)
end)
CarSubOffRoad:add_action("Spawn Insurgent", function()
	SpawnVehicle(-1860900134)
end)
CarSubOffRoad:add_action("Spawn Insurgent2", function()
	SpawnVehicle(2071877360)
end)
CarSubOffRoad:add_action("Spawn Insurgent3", function()
	SpawnVehicle(-1924433270)
end)
CarSubOffRoad:add_action("Spawn Kalahari", function()
	SpawnVehicle(92612664)
end)
CarSubOffRoad:add_action("Spawn Lifeguard", function()
	SpawnVehicle(469291905)
end)
CarSubOffRoad:add_action("Spawn Marshall", function()
	SpawnVehicle(1233534620)
end)
CarSubOffRoad:add_action("Spawn Mesa", function()
	SpawnVehicle(914654722)
end)
CarSubOffRoad:add_action("Spawn Mesa2", function()
	SpawnVehicle(-748008636)
end)
CarSubOffRoad:add_action("Spawn Mesa3", function()
	SpawnVehicle(-2064372143)
end)
CarSubOffRoad:add_action("Spawn Monster", function()
	SpawnVehicle(-845961253)
end)
CarSubOffRoad:add_action("Spawn Nightshark", function()
	SpawnVehicle(433954513)
end)
CarSubOffRoad:add_action("Spawn RancherXL", function()
	SpawnVehicle(-2064372143)
end)
CarSubOffRoad:add_action("Spawn RancherXL2", function()
	SpawnVehicle(-845961253)
end)
CarSubOffRoad:add_action("Spawn Rebel", function()
	SpawnVehicle(-1207771834)
end)
CarSubOffRoad:add_action("Spawn Rebel2", function()
	SpawnVehicle(-2045594037)
end)
CarSubOffRoad:add_action("Spawn Sandking", function()
	SpawnVehicle(-1189015600)
end)
CarSubOffRoad:add_action("Spawn Sandking2", function()
	SpawnVehicle(989381445)
end)
CarSubOffRoad:add_action("Spawn Technical", function()
	SpawnVehicle(-2096818938)
end)
CarSubOffRoad:add_action("Spawn Technical2", function()
	SpawnVehicle(1180875963)
end)
CarSubOffRoad:add_action("Spawn Technical3", function()
	SpawnVehicle(1356124575)
end)
CarSubOffRoad:add_action("Spawn TrophyTruck", function()
	SpawnVehicle(101905590)
end)
CarSubOffRoad:add_action("Spawn TrophyTruck2", function()
	SpawnVehicle(-663299102)
end)
...
CarSubPlanes:add_action("Spawn Blimp2", function()
	SpawnVehicle(-613725916)
end)
CarSubPlanes:add_action("Spawn CargoPlane", function()
	SpawnVehicle(368211810)
end)
CarSubPlanes:add_action("Spawn Cuban800", function()
	SpawnVehicle(-644710429)
end)
CarSubPlanes:add_action("Spawn Dodo", function()
	SpawnVehicle(-901163259)
end)
CarSubPlanes:add_action("Spawn Duster", function()
	SpawnVehicle(-970356638)
end)
CarSubPlanes:add_action("Spawn Hydra", function()
	SpawnVehicle(970385471)
end)
CarSubPlanes:add_action("Spawn Jet", function()
	SpawnVehicle(1058115860)
end)
CarSubPlanes:add_action("Spawn Lazer", function()
	SpawnVehicle(-1281684762)
end)
CarSubPlanes:add_action("Spawn Luxor", function()
	SpawnVehicle(621481054)
end)
CarSubPlanes:add_action("Spawn Luxor2", function()
	SpawnVehicle(-1214293858)
end)
CarSubPlanes:add_action("Spawn Mammatus", function()
	SpawnVehicle(-1746576111)
end)
CarSubPlanes:add_action("Spawn Miljet", function()
	SpawnVehicle(165154707)
end)
CarSubPlanes:add_action("Spawn Nimbus", function()
	SpawnVehicle(-1295027632)
end)
CarSubPlanes:add_action("Spawn Shamal", function()
	SpawnVehicle(-1214505995)
end)
CarSubPlanes:add_action("Spawn Stunt", function()
	SpawnVehicle(-2122757008)
end)
CarSubPlanes:add_action("Spawn Titan", function()
	SpawnVehicle(1981688531)
end)
CarSubPlanes:add_action("Spawn Velum", function()
	SpawnVehicle(-1673356438)
end)
CarSubPlanes:add_action("Spawn Velum2", function()
	SpawnVehicle(1077420264)
end)
CarSubPlanes:add_action("Spawn Vestra", function()
	SpawnVehicle(1341619767)
end)

-- Sedans
CarSubSedans:add_action("Spawn Asea", function()
	SpawnVehicle(-1809822327)
end)
CarSubSedans:add_action("Spawn Asea2", function()
	SpawnVehicle(-1807623979)
end)
CarSubSedans:add_action("Spawn Asterope", function()
	SpawnVehicle(-1903012613)
end)
CarSubSedans:add_action("Spawn Cog55", function()
	SpawnVehicle(906642318)
end)
CarSubSedans:add_action("Spawn Cog552", function()
	SpawnVehicle(704435172)
end)
CarSubSedans:add_action("Spawn Cognoscenti", function()
	SpawnVehicle(-2030171296)
end)
CarSubSedans:add_action("Spawn Cognoscenti2", function()
	SpawnVehicle(-604842630)
end)
CarSubSedans:add_action("Spawn Emperor", function()
	SpawnVehicle(-685276541)
end)
CarSubSedans:add_action("Spawn Emperor2", function()
	SpawnVehicle(-1883002148)
end)
CarSubSedans:add_action("Spawn Emperor3", function()
	SpawnVehicle(-1241712818)
end)
CarSubSedans:add_action("Spawn Fugitive", function()
	SpawnVehicle(1909141499)
end)
CarSubSedans:add_action("Spawn Glendale", function()
	SpawnVehicle(75131841)
end)
CarSubSedans:add_action("Spawn Ingot", function()
	SpawnVehicle(-1289722222)
end)
CarSubSedans:add_action("Spawn Intruder", function()
	SpawnVehicle(886934177)
end)
CarSubSedans:add_action("Spawn Limo2", function()
	SpawnVehicle(-114627507)
end)
CarSubSedans:add_action("Spawn Premier", function()
	SpawnVehicle(-1883869285)
end)
CarSubSedans:add_action("Spawn Primo", function()
	SpawnVehicle(-1150599089)
end)
CarSubSedans:add_action("Spawn Primo2", function()
	SpawnVehicle(-2040426790)
end)
CarSubSedans:add_action("Spawn Regina", function()
	SpawnVehicle(-14495224)
end)
CarSubSedans:add_action("Spawn Romero", function()
	SpawnVehicle(627094268)
end)
CarSubSedans:add_action("Spawn Rhinehart", function()
	SpawnVehicle(1855505138)
end)
CarSubSedans:add_action("Spawn Stanier", function()
	SpawnVehicle(-1477580979)
end)
CarSubSedans:add_action("Spawn Stratum", function()
	SpawnVehicle(1723137093)
end)
CarSubSedans:add_action("Spawn Stretch", function()
	SpawnVehicle(-1961627517)
end)
CarSubSedans:add_action("Spawn Surge", function()
	SpawnVehicle(-1894894188)
end)
CarSubSedans:add_action("Spawn Tailgater", function()
	SpawnVehicle(-1008861746)
end)
CarSubSedans:add_action("Spawn Warrener", function()
	SpawnVehicle(1373123368)
end)
CarSubSedans:add_action("Spawn Washington", function()
	SpawnVehicle(1777363799)
end)

-- Service
CarSubService:add_action("Spawn Airbus", function()
	SpawnVehicle(1283517198)
end)
CarSubService:add_action("Spawn Brickade", function()
	SpawnVehicle(-305727417)
end)
CarSubService:add_action("Spawn Brickade2 | (Acid Lab)", function()
	SpawnVehicle(-1576586413)
end)
CarSubService:add_action("Spawn Bus", function()
	SpawnVehicle(-713569950)
end)
CarSubService:add_action("Spawn Coach", function()
	SpawnVehicle(-2072933068)
end)
CarSubService:add_action("Spawn Rallytruck", function()
	SpawnVehicle(-2103821244)
end)
CarSubService:add_action("Spawn RentalBus", function()
	SpawnVehicle(-1098802077)
end)
CarSubService:add_action("Spawn Taxi", function()
	SpawnVehicle(-956048545)
end)
CarSubService:add_action("Spawn Tourbus", function()
	SpawnVehicle(1941029835)
end)
CarSubService:add_action("Spawn Trash", function()
	SpawnVehicle(1917016601)
end)
CarSubService:add_action("Spawn Trash2", function()
	SpawnVehicle(-1255698084)
end)

-- Sports
CarSubSports:add_action("Spawn 300R", function()
	SpawnVehicle(1076201208)
end)
CarSubSports:add_action("Spawn Alpha", function()
	SpawnVehicle(767087018)
end)
CarSubSports:add_action("Spawn Banshee", function()
	SpawnVehicle(-1041692462)
end)
CarSubSports:add_action("Spawn Banshee2", function()
	SpawnVehicle(633712403)
end)
CarSubSports:add_action("Spawn BestiaGTS", function()
	SpawnVehicle(1274868363)
end)
CarSubSports:add_action("Spawn Buffalo", function()
	SpawnVehicle(-304802106)
end)
CarSubSports:add_action("Spawn Buffalo2", function()
	SpawnVehicle(736902334)
end)
CarSubSports:add_action("Spawn Buffalo3", function()
	SpawnVehicle(237764926)
end)
CarSubSports:add_action("Spawn Carbonizzare", function()
	SpawnVehicle(2072687711)
end)
CarSubSports:add_action("Spawn Comet2", function()
	SpawnVehicle(-1045541610)
end)
CarSubSports:add_action("Spawn Comet3", function()
	SpawnVehicle(-2022483795)
end)
CarSubSports:add_action("Spawn Corsita", function()
	SpawnVehicle(754687673)
end)
CarSubSports:add_action("Spawn Coquette", function()
	SpawnVehicle(108773431)
end)
CarSubSports:add_action("Spawn Elegy", function()
	SpawnVehicle(196747873)
end)
CarSubSports:add_action("Spawn Elegy2", function()
	SpawnVehicle(-566387422)
end)
CarSubSports:add_action("Spawn Entity", function()
	SpawnVehicle(1748565021)
end)
CarSubSports:add_action("Spawn Everon", function()
	SpawnVehicle(-131348178)
end)
CarSubSports:add_action("Spawn Feltzer2", function()
	SpawnVehicle(-1995326987)
end)
CarSubSports:add_action("Spawn Feltzer3", function()
	SpawnVehicle(-1566741232)
end)
CarSubSports:add_action("Spawn Furoregt", function()
	SpawnVehicle(-1089039904)
end)
CarSubSports:add_action("Spawn Fusilade", function()
	SpawnVehicle(499169875)
end)
CarSubSports:add_action("Spawn Futo", function()
	SpawnVehicle(2016857647)
end)
CarSubSports:add_action("Spawn Infernus2", function()
	SpawnVehicle(-1405937764)
end)
CarSubSports:add_action("Spawn Jester", function()
	SpawnVehicle(-1297672541)
end)
CarSubSports:add_action("Spawn Jester2", function()
	SpawnVehicle(-1106353882)
end)
CarSubSports:add_action("Spawn Khamelion", function()
	SpawnVehicle(544021352)
end)
CarSubSports:add_action("Spawn Kuruma", function()
	SpawnVehicle(-1372848492)
end)
CarSubSports:add_action("Spawn ArmoredKuruma", function()
	SpawnVehicle(410882957)
end)
CarSubSports:add_action("Spawn LM87", function()
	SpawnVehicle(10917683)
end)
CarSubSports:add_action("Spawn Lynx", function()
	SpawnVehicle(482197771)
end)
CarSubSports:add_action("Spawn Massacro", function()
	SpawnVehicle(-142942670)
end)
CarSubSports:add_action("Spawn Massacro2", function()
	SpawnVehicle(-631760477)
end)
CarSubSports:add_action("Spawn Ninef", function()
	SpawnVehicle(1032823388)
end)
CarSubSports:add_action("Spawn Ninef2", function()
	SpawnVehicle(-1461482751)
end)
CarSubSports:add_action("Spawn tenf", function()
	SpawnVehicle(893984159)
end)
CarSubSports:add_action("Spawn tenf2", function()
	SpawnVehicle(274946574)
end)
CarSubSports:add_action("Spawn Omnis", function()
	SpawnVehicle(-777172681)
end)
CarSubSports:add_action("Spawn Omnisegt", function()
	SpawnVehicle(505223465)
end)
CarSubSports:add_action("Spawn Panthere", function()
	SpawnVehicle(2100457220)
end)
CarSubSports:add_action("Spawn Penumbra", function()
	SpawnVehicle(-377465520)
end)
CarSubSports:add_action("Spawn RapidGT", function()
	SpawnVehicle(-1934452204)
end)
CarSubSports:add_action("Spawn RapidGT2", function()
	SpawnVehicle(1737773231)
end)
CarSubSports:add_action("Spawn Raptor", function()
	SpawnVehicle(-674927303)
end)
CarSubSports:add_action("Spawn Ruston", function()
	SpawnVehicle(719660200)
end)
CarSubSports:add_action("Spawn Schafter2", function()
	SpawnVehicle(-1255452397)
end)
CarSubSports:add_action("Spawn Schafter3", function()
	SpawnVehicle(-1485523546)
end)
CarSubSports:add_action("Spawn Schafter4", function()
	SpawnVehicle(1489967196)
end)
CarSubSports:add_action("Spawn Schafter5", function()
	SpawnVehicle(-888242983)
end)
CarSubSports:add_action("Spawn Schafter6", function()
	SpawnVehicle(1922255844)
end)
CarSubSports:add_action("Spawn Schwarzer", function()
	SpawnVehicle(-746882698)
end)
CarSubSports:add_action("Spawn Seven70", function()
	SpawnVehicle(-1757836725)
end)
CarSubSports:add_action("Spawn Specter", function()
	SpawnVehicle(1886268224)
end)
CarSubSports:add_action("Spawn Specter2", function()
	SpawnVehicle(1074745671)
end)
CarSubSports:add_action("Spawn Sultan", function()
	SpawnVehicle(970598228)
end)
CarSubSports:add_action("Spawn Surano", function()
	SpawnVehicle(384071873)
end)
CarSubSports:add_action("Spawn sm722", function()
	SpawnVehicle(775514032)
end)
CarSubSports:add_action("Spawn Tampa2", function()
	SpawnVehicle(-1071380347)
end)
CarSubSports:add_action("Spawn Torero", function()
	SpawnVehicle(165394758)
end)
CarSubSports:add_action("Spawn Tropos", function()
	SpawnVehicle(1887331236)
end)
CarSubSports:add_action("Spawn Verlierer2", function()
	SpawnVehicle(1102544804)
end)
CarSubSports:add_action("Spawn Virtue", function()
	SpawnVehicle(669204833)
end)

-- SUV
CarSubSUV:add_action("Spawn BJXL", function()
	SpawnVehicle(850565707)
end)
CarSubSUV:add_action("Spawn Baller", function()
	SpawnVehicle(-808831384)
end)
CarSubSUV:add_action("Spawn Baller2", function()
	SpawnVehicle(142944341)
end)
CarSubSUV:add_action("Spawn Baller3", function()
	SpawnVehicle(1878062887)
end)
CarSubSUV:add_action("Spawn Baller4", function()
	SpawnVehicle(634118882)
end)
CarSubSUV:add_action("Spawn Baller5", function()
	SpawnVehicle(470404958)
end)
CarSubSUV:add_action("Spawn Baller6", function()
	SpawnVehicle(666166960)
end)
CarSubSUV:add_action("Spawn Cavalcade", function()
	SpawnVehicle(2006918058)
end)
CarSubSUV:add_action("Spawn Cavalcade2", function()
	SpawnVehicle(-789894171)
end)
CarSubSUV:add_action("Spawn Contender", function()
	SpawnVehicle(683047626)
end)
CarSubSUV:add_action("Spawn Dubsta", function()
	SpawnVehicle(1177543287)
end)
CarSubSUV:add_action("Spawn Dubsta2", function()
	SpawnVehicle(-394074634)
end)
CarSubSUV:add_action("Spawn Dubsta3", function()
	SpawnVehicle(-1237253773)
end)
CarSubSUV:add_action("Spawn FQ2", function()
	SpawnVehicle(-1137532101)
end)
CarSubSUV:add_action("Spawn Granger", function()
	SpawnVehicle(-1775728740)
end)
CarSubSUV:add_action("Spawn Gresley", function()
	SpawnVehicle(-1543762099)
end)
CarSubSUV:add_action("Spawn Habanero", function()
	SpawnVehicle(884422927)
end)
CarSubSUV:add_action("Spawn Huntley", function()
	SpawnVehicle(486987393)
end)
CarSubSUV:add_action("Spawn Issi", function()
	SpawnVehicle(1550581940)
end)
CarSubSUV:add_action("Spawn Landstalker", function()
	SpawnVehicle(1269098716)
end)
CarSubSUV:add_action("Spawn Patriot", function()
	SpawnVehicle(-808457413)
end)
CarSubSUV:add_action("Spawn Radi", function()
	SpawnVehicle(-1651067813)
end)
CarSubSUV:add_action("Spawn Rocoto", function()
	SpawnVehicle(2136773105)
end)
CarSubSUV:add_action("Spawn Seminole", function()
	SpawnVehicle(1221512915)
end)
CarSubSUV:add_action("Spawn Serrano", function()
	SpawnVehicle(1337041428)
end)
CarSubSUV:add_action("Spawn XLS", function()
	SpawnVehicle(1203490606)
end)
CarSubSUV:add_action("Spawn XLS2", function()
	SpawnVehicle(-432008408)
end)

-- Utility
CarSubUtility:add_action("Spawn Airtug", function()
	SpawnVehicle(1560980623)
end)
CarSubUtility:add_action("Spawn Caddy", function()
	SpawnVehicle(1147287684)
end)
CarSubUtility:add_action("Spawn Caddy2", function()
	SpawnVehicle(-537896628)
end)
CarSubUtility:add_action("Spawn Caddy3", function()
	SpawnVehicle(-769147461)
end)
CarSubUtility:add_action("Spawn Docktug", function()
	SpawnVehicle(-884690486)
end)
CarSubUtility:add_action("Spawn Forklift", function()
	SpawnVehicle(1491375716)
end)
CarSubUtility:add_action("Spawn Mower", function()
	SpawnVehicle(1783355638)
end)
CarSubUtility:add_action("Spawn Ripley", function()
	SpawnVehicle(-845979911)
end)
CarSubUtility:add_action("Spawn Sadler", function()
	SpawnVehicle(-599568815)
end)
CarSubUtility:add_action("Spawn Scrap", function()
	SpawnVehicle(-1700801569)
end)
CarSubUtility:add_action("Spawn TowTruck", function()
	SpawnVehicle(-1323100960)
end)
CarSubUtility:add_action("Spawn TowTruck2", function()
	SpawnVehicle(-442313018)
end)
CarSubUtility:add_action("Spawn Tractor", function()
	SpawnVehicle(1641462412)
end)
CarSubUtility:add_action("Spawn Tractor2", function()
	SpawnVehicle(-2076478498)
end)
CarSubUtility:add_action("Spawn Tractor3", function()
	SpawnVehicle(1445631933)
end)
CarSubUtility:add_action("Spawn TrailerLarge", function()
	SpawnVehicle(1502869817)
end)
CarSubUtility:add_action("Spawn TrailerS4", function()
	SpawnVehicle(-1100548694)
end)
CarSubUtility:add_action("Spawn UtilliTruck", function()
	SpawnVehicle(516990260)
end)
CarSubUtility:add_action("Spawn UtilliTruck2", function()
	SpawnVehicle(887537515)
end)
CarSubUtility:add_action("Spawn UtilliTruck3", function()
	SpawnVehicle(2132890591)
end)

-- Vans
CarSubVans:add_action("Spawn Bison", function()
	SpawnVehicle(-16948145)
end)
CarSubVans:add_action("Spawn Bison2", function()
	SpawnVehicle(2072156101)
end)
CarSubVans:add_action("Spawn Bison3", function()
	SpawnVehicle(1739845664)
end)
CarSubVans:add_action("Spawn BobcatXL", function()
	SpawnVehicle(1069929536)
end)
CarSubVans:add_action("Spawn Boxville", function()
	SpawnVehicle(-1987130134)
end)
CarSubVans:add_action("Spawn Boxville2", function()
	SpawnVehicle(-233098306)
end)
CarSubVans:add_action("Spawn Boxville3", function()
	SpawnVehicle(121658888)
end)
CarSubVans:add_action("Spawn Boxville4", function()
	SpawnVehicle(444171386)
end)
CarSubVans:add_action("Spawn Boxville5", function()
	SpawnVehicle(682434785)
end)
CarSubVans:add_action("Spawn Burrito", function()
	SpawnVehicle(-1346687836)
end)
CarSubVans:add_action("Spawn Burrito2", function()
	SpawnVehicle(-907477130)
end)
CarSubVans:add_action("Spawn Burrito3", function()
	SpawnVehicle(-1743316013)
end)
CarSubVans:add_action("Spawn Burrito4", function()
	SpawnVehicle(893081117)
end)
CarSubVans:add_action("Spawn Burrito5", function()
	SpawnVehicle(1132262048)
end)
CarSubVans:add_action("Spawn Camper", function()
	SpawnVehicle(1876516712)
end)
CarSubVans:add_action("Spawn GBurrito", function()
	SpawnVehicle(-1745203402)
end)
CarSubVans:add_action("Spawn GBurrito2", function()
	SpawnVehicle(296357396)
end)
CarSubVans:add_action("Spawn Journey", function()
	SpawnVehicle(296357396)
end)
CarSubVans:add_action("Spawn Journey2", function()
	SpawnVehicle(-1627077503)
end)
CarSubVans:add_action("Spawn Minivan", function()
	SpawnVehicle(-310465116)
end)
CarSubVans:add_action("Spawn Minivan2", function()
	SpawnVehicle(-1126264336)
end)
CarSubVans:add_action("Spawn Paradise", function()
	SpawnVehicle(1488164764)
end)
CarSubVans:add_action("Spawn Pony", function()
	SpawnVehicle(-119658072)
end)
CarSubVans:add_action("Spawn Pony2", function()
	SpawnVehicle(943752001)
end)
CarSubVans:add_action("Spawn Rumpo", function()
	SpawnVehicle(1162065741)
end)
CarSubVans:add_action("Spawn Rumpo2", function()
	SpawnVehicle(-1776615689)
end)
CarSubVans:add_action("Spawn Rumpo3", function()
	SpawnVehicle(1475773103)
end)
CarSubVans:add_action("Spawn Speedo", function()
	SpawnVehicle(-810318068)
end)
CarSubVans:add_action("Spawn Speedo2", function()
	SpawnVehicle(728614474)
end)
CarSubVans:add_action("Spawn Surfer", function()
	SpawnVehicle(699456151)
end)
CarSubVans:add_action("Spawn Surfer2", function()
	SpawnVehicle(-1311240698)
end)
CarSubVans:add_action("Spawn Surfer3", function()
	SpawnVehicle(-1035489563)
end)
CarSubVans:add_action("Spawn Taco", function()
	SpawnVehicle(1951180813)
end)
CarSubVans:add_action("Spawn Youga", function()
	SpawnVehicle(65402552)
end)
CarSubVans:add_action("Spawn Youga2", function()
	SpawnVehicle(1026149675)
end)

-- Commercial
CarSubCommercial:add_action("Spawn Benson", function()
	SpawnVehicle(2053223216)
end)
CarSubCommercial:add_action("Spawn Biff", function()
	SpawnVehicle(850991848)
end)
CarSubCommercial:add_action("Spawn Hauler", function()
	SpawnVehicle(1518533038)
end)
CarSubCommercial:add_action("Spawn Hauler2", function()
	SpawnVehicle(387748548)
end)
CarSubCommercial:add_action("Spawn Mule", function()
	SpawnVehicle(904750859)
end)
CarSubCommercial:add_action("Spawn Mule2", function()
	SpawnVehicle(-1050465301)
end)
CarSubCommercial:add_action("Spawn Mule3", function()
	SpawnVehicle(-2052737935)
end)
CarSubCommercial:add_action("Spawn Packer", function()
	SpawnVehicle(569305213)
end)
CarSubCommercial:add_action("Spawn Phantom", function()
	SpawnVehicle(-2137348917)
end)
CarSubCommercial:add_action("Spawn Phantom2", function()
	SpawnVehicle(-1649536104)
end)
CarSubCommercial:add_action("Spawn Phantom3", function()
	SpawnVehicle(177270108)
end)
CarSubCommercial:add_action("Spawn Phantom Christmas", function()
	SpawnVehicle(-129283887)
end)
CarSubCommercial:add_action("Spawn Trailer Christmas", function()
	SpawnVehicle(-1334453816)
end)
CarSubCommercial:add_action("Spawn Pounder", function()
	SpawnVehicle(2112052861)
end)
CarSubCommercial:add_action("Spawn Stockade", function()
	SpawnVehicle(1747439474)
end)
CarSubCommercial:add_action("Spawn Stockade2", function()
	SpawnVehicle(-214455498)
end)
...
Peyote:add_array_item("Location No.", {
	"01",
	"02",
	"03",
	"04",
...
Lantern:add_array_item("Location No.", {
	"01",
	"02",
	"03",
	"04",
...
CharMenu:add_toggle("Max Stats", function()
	...
	if
		Get(MPX .. "STAMINA") == 100
		and Get(MPX .. "SHOOTING_ABILITY") == 100
		and Get(MPX .. "SHOOTING_ABILITY")
		and Get(MPX .. "STRENGTH") == 100
		and Get(MPX .. "STEALTH_ABILITY") == 100
		and Get(MPX .. "FLYING_ABILITY") == 100
		and Get(MPX .. "WHEELIE_ABILITY") == 100
		and Get(MPX .. "LUNG_CAPACITY") == 100
	then
		return true
	else
		return false
...
CharMenu:add_bare_item("", function()
	...
	elseif stamina > 50 and stamina <= 75 then
		status = "(athlete)"
	elseif stamina > 75 and stamina <= 99 then
		status = "(pro)"
	else
		status = "(Usain Bolt)"
	end
...
CharMenu:add_bare_item("", function()
	...
	elseif shooting > 50 and shooting <= 75 then
		status = "(policeman)"
	elseif shooting > 75 and shooting <= 99 then
		status = "(pvp kid)"
	else
		status = "(John Wick)"
	end
...
CharMenu:add_bare_item("", function()
	...
	elseif strength > 50 and strength <= 75 then
		status = "(boxer)"
	elseif strength > 75 and strength <= 99 then
		status = "(builder)"
	else
		status = "(Gym Rat)"
	end
...
CharMenu:add_bare_item("", function()
	...
	elseif stealth > 50 and stealth <= 75 then
		status = "(thief)"
	elseif stealth > 75 and stealth <= 99 then
		status = "(assassin)"
	else
		status = "(Agent 47)"
	end
...
CharMenu:add_bare_item("", function()
	...
	elseif flying > 50 and flying <= 75 then
		status = "(pilot)"
	elseif flying > 75 and flying <= 99 then
		status = "(war thunder enjoyer)"
	else
		status = "(«Maverick» Mitchell)"
	end
...
CharMenu:add_bare_item("", function()
	...
	elseif driving > 50 and driving <= 75 then
		status = "(taxi driver)"
	elseif driving > 75 and driving <= 99 then
		status = "(drifter)"
	else
		status = "(Ken Block)"
	end
...
CharMenu:add_bare_item("", function()
	...
	elseif swimming > 50 and swimming <= 75 then
		status = "(fisherman)"
	elseif swimming > 75 and swimming <= 99 then
		status = "(dolphine)"
	else
		status = "(Poseidon)"
	end
